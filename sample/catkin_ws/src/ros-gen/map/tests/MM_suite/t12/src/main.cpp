...
int main(int argc, char** argv) {
	...
	auto trace = builder.then(E(inpIN,1)).then(E(outpOUT,Value::Low())).then(E(outpOUT,Value::High()));
	...
	checker = checker.initially(test.getMessages()).then(E(inpIN,1)).pass(E(outpOUT,Value::Low())).forbid(E(outpOUT,Value::High()));
