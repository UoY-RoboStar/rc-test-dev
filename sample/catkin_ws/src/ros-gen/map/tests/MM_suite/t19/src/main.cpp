...
int main(int argc, char** argv) {
	...
	auto trace = builder.then(E(inpIN,2)).then(E(outpOUT,Value::Low())).then(E(outpOUT,Value::Low()));
	...
	checker = checker.initially(test.getMessages()).then(E(inpIN,2)).pass(E(outpOUT,Value::Low())).forbid(E(outpOUT,Value::Low()));
